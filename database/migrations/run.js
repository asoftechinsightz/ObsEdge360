...
async function main() {
  ...
  const p4rc2 = await tableExists(client, 'rc2_readiness');
  if (!p4rc2) {
    const s45 = path.join(migrationsDir, '045_rc2_pilot_production_readiness.sql');
    if (fs.existsSync(s45)) await runSqlFile(client, s45);
  } else {
